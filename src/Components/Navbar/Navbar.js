    ...
    return (
        <div className='d-flex flex-row justify-content-center nav-main'>
            ...
            <li><Link className='nav-item' to="/">Home</Link></li>
                ...
                {
                        loggedInUser.email ? <p className='btn btn-info' onClick={logOut}>Log Out</p> : <p><Link className='btn btn-warning' to="/login">Login</Link></p>
                }
                {
                    loggedInUser.email ? <p><Link className='nav-item btn btn-success ml-3'>{loggedInUser.name}</Link></p> : <p></p>
                }
            </ul>
        </div>
    );
